function weather -d "Displays weather info"
  ...
  if test -z $loc
    ...
    if not set json (weather.fetch "https://api.openweathermap.org/data/2.5/weather" lat=$location[1] lon=$location[2] APPID=$api_key)
      echo "Unable to fetch weather data; please try again later."
      return 1
    end
  else
    # Fetch weather based on a search query.
    if not set json (weather.fetch "https://api.openweathermap.org/data/2.5/weather" "q=$loc" APPID=$api_key)
      echo "Unable to fetch weather data; please try again later."
      return 1
    end
